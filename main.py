...
class AllSprites(pygame.sprite.Group):
    def __init__(self):
        ...
        self.bg = pygame.image.load('progress/project_3 - player_movement/graphics/main/map.png').convert()
        self.fg = pygame.image.load('progress/project_3 - player_movement/graphics/main/overlay.png').convert_alpha()
    ...
    def customize_draw(self):
        ...
        for sprite in sorted(self.sprites(),key = lambda sprite: sprite.rect.centery): # if player is below the car it will now look like the player is in front
...
# music
music = pygame.mixer.Sound('progress/project_1 - setup/audio/music.mp3')
music.play(loops = -1)

# sprite setup
for file_name, pos_list in SIMPLE_OBJECTS.items():
    path = f'progress/project_1 - setup/graphics/objects/simple/{file_name}.png'
    surf = pygame.image.load(path).convert_alpha()
    for pos in pos_list:
        SimpleSprite(surf,pos,[all_sprites, obstacle_sprites])
        
for file_name, pos_list in LONG_OBJECTS.items():
    surf = pygame.image.load(f'progress/project_1 - setup/graphics/objects/long/{file_name}.png').convert_alpha()
    for pos in pos_list:
